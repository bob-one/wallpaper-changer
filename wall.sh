...
set_wallpaper () {
    ...
    WALLPAPER=$( awk '{if(NR=="'"$RAND"'") print$0}' $SCRIPT_PATH/list_of_wallpapers)
    feh --bg-scale $WALLPAPER 2>>$SCRIPT_PATH/wallpaper_error
...
start_over () {
    # reset INDX and RAND
    INDX=1; RAND=1
    echo $INDX > $SCRIPT_PATH/current_wallpaper_index
    ...
    echo $NUMPICS > $SCRIPT_PATH/number_of_pics # For use in the main loop
    ...
    echo "" > $SCRIPT_PATH/list_of_wallpapers
    for f in "$WALLPAPER_PATH"/*
    do
        echo "$f" >> $SCRIPT_PATH/list_of_wallpapers
    done
...
cycle_through () {
    ...
    INDX=$(( INDX+1 ))
    echo $INDX > $SCRIPT_PATH/current_wallpaper_index
    RAND=$( head -n 1 $SCRIPT_PATH/list_of_non_repeating_random_numbers )
    sed -i -e "1d" $SCRIPT_PATH/list_of_non_repeating_random_numbers
    set_wallpaper $RAND 
}
...
read INDX < $SCRIPT_PATH/current_wallpaper_index 2>>$SCRIPT_PATH/wallpaper_error
[[ "$INDX" = "reset" ]] && ( start_over )
read LIMIT < $SCRIPT_PATH/number_of_pics 2>>$SCRIPT_PATH/wallpaper_error
